...
def calculate_gaps(samples: dict) -> tuple[np.ndarray, np.ndarray]:
    ...
    # Calculate and print distribution of g_SC + t_sat/12
    total_time = g_SC + samples["t_sat"]
    print("\nDistribution of g_SC + t_sat:")
    ...
    for i in tqdm(range(len(samples["T_t"])), desc="Running simulations", leave=False):
        ...
        for _ in range(params["n_steps"]):
            ...
            if t >= max_time:
                ...
                break
    # Calculate average successful time for samples with t_sat/12 + g_SC between 55-65 months
    t_sat_plus_g_SC = samples["t_sat"] + g_SC
    mask_55_65 = (t_sat_plus_g_SC >= 55) & (t_sat_plus_g_SC <= 65)
    if np.any(mask_55_65):
        avg_successful_time_55_65 = np.mean(np.array(successful_times)[mask_55_65])
        print(f"\nAverage successful time for samples with t_sat + g_SC 55-65 months: {avg_successful_time_55_65:.2f} years")
        print(f"Number of samples in this range: {np.sum(mask_55_65)}")
    else:
        print("\nNo samples found with t_sat + g_SC between 55-65 months")
