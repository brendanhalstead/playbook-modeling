...
def calculate_sc_arrival_year_with_trajectories(samples: dict, current_horizon: float, dt: float, compute_decrease_date: float, human_alg_progress_decrease_date: float, max_simulation_years: float) -> tuple[np.ndarray, list]:
    ...
    # Calculate average ending time for samples with base_time_in_months between 55-65
    mask_55_65 = (base_time_in_months >= 55) & (base_time_in_months <= 65)
    if np.any(mask_55_65):
        avg_ending_time_55_65 = np.mean(ending_times[mask_55_65])
        print(f"\nAverage ending time for samples with base time 55-65 months: {avg_ending_time_55_65:.2f} years")
        print(f"Number of samples in this range: {np.sum(mask_55_65)}")
    else:
        print("\nNo samples found with base time between 55-65 months")
