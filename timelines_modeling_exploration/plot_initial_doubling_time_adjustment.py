...
def compute_superexponential_curve(
    ...
    Returns:
        x_2024_years: array of 2024-years values (starting at 0)
        horizons: array of horizon values in minutes
        singularity_2024_years: 2024-years when singularity occurs
    """
    q = 1 - doubling_decay_fraction

    # Singularity time in months
    singularity_time_months = initial_doubling_time_months / (1 - q)
    # Convert to 2024-years (months / 12 gives calendar years, which equals 2024-years)
    singularity_2024_years = singularity_time_months / 12
    ...
    # Convert to 2024-years (starting at 0)
    x_2024_years = time_months / 12
    ...
    # Don't clip - let the curve go to infinity near singularity
    # The y-axis limits will handle the visual clipping

    return x_2024_years, horizons, singularity_2024_years
...
def main():
    ...
    # Compute curves (x-axis is now 2024-years starting at 0)
    ...
    # Formatting
    ax.set_yscale('log')
    ax.set_xlabel('The factor by which log(effective compute) has increased, relative to the amount it increased in 2024\n(elsewhere called 2024-years)', fontsize=12)
    ...
    # Set x-axis limits
    ax.set_xlim(-0.1, max(sing_a, sing_b, sing_c) + 0.5)
    ...
    for arrow_y, extra_label in zip(arrow_y_values, arrow_labels):
        ...
        # Calculate the actual shift at this horizon
        actual_shift = x_green - x_blue

        # Only draw if both points are within plot bounds
        if x_blue >= 0 and x_green <= max(sing_a, sing_b, sing_c) + 0.5:
            ...
            ax.text(mid_x, arrow_y * 1.5, f'{actual_shift:.1f} 2024-years {extra_label}',
    ...
    add_disclaimer(fig)
    plt.savefig('timelines_modeling_exploration/initial_doubling_time_adjustment.png', dpi=150, bbox_inches='tight')
    # plt.show()  # Commented out to not block

    print(f"\nPlot saved to timelines_modeling_exploration/initial_doubling_time_adjustment.png")
